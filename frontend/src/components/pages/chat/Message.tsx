...
export const Message: React.FC<Props> = ({ message, showSeenTime }) => {
  ...
  const containerClass = classnames("w-[72%] shadow-md flex flex-col relative mb-4 flex-shrink-0 rounded-lg", {
  ...
  return (
    <div className={containerClass}>
      <div className={headClass} title={message.senderInfo?.name}>
        ...
        <span className="opacity-50">{prettifyDate(message.createdAt)}</span>
      </div>
